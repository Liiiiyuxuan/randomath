...
f = @(x, y) sin(x) * y * sin(y^2);  % Updated function dy/dx = sin(x) * sin(y^2)

% Set the domain for the directional field
xrange = linspace(-10, 10, 20);  % 20 points from -10 to 10
yrange = linspace(-10, 10, 20);
...
xlabel('x');
ylabel('y');
title('Directional Field for dy/dx = sin(x) * sin(y^2)');
