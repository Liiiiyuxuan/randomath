...
\begin{solution}
    Suppose for a contradiction that $L$ is regular. Hence for a fixed pumping length $p$, we can find $\ell \in L$ with $|\ell| \geq p$ such that $\ell = xyz$. We assume that $|\ell| = $ By the pumping lemma, we know that 
\end{solution}
