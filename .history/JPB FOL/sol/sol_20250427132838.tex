...
\begin{solution}
    Similar to exercise \ref{exercise5.4} with the use of Fermat's Little Theorem. Suppose for a contradiction that the set $S$ is regular, thus it needs to satisfy the pumping lemma for some pumping length $p$. Find a prime $\gamma$ such that its base-$k$ expansion has length at least $p$, and can be written as $\gamma = uvw$, 
\end{solution}
