...
\begin{solution}
    Suppose for a contradiction that the set of words over the alphabet $\{0, 1, \ldots, k-1\}$ that represent the base-$k$ expansions of elements of $\{\ell^n : n \geq 0\}$ does form a regular language. We  Hence for the pumping length $p$, let $\ell^n$ be an element whose base-$k$ expansion has length at least $p$, suppose $\ell^n = uvw$, we have 
