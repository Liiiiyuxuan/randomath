...
\begin{solution}
    This is similar to exercise 5.1. Suppose for a contradiction that the set of palindromes is regular, then there exists a pumping length $p$ satisfying the pumping lemma. Now consider an arbitrary string (palindrome) of length $2p + 1$, (which is indeed length of at least $p$ since we have $p \geq 1$), we know that 
\end{solution}
