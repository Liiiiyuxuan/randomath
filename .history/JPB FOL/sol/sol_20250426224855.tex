...
\begin{solution}
    Since we have the convention that $(0)_k$ is the empty word, we have the following automoton accepting  \begin{center}
        \begin{tikzpicture}[shorten >=1pt, node distance=2.5cm, on grid, auto]
            ...
            (q1) edge[loop above] node {$0,\dots,k-1$} ()
            ;
        \end{tikzpicture}
    \end{center}
\end{solution}
...
\begin{solution}
    ...
    Thus we can extend this definition. Suppose we have $u = u_h u_{h-1} \cdots u_0$, then 
    \begin{align*}
        [ u v^0 \omega ]_k = [ u \omega ]_k 
        & = u_h k^{h+m+1} + u_{h-1} k^{h+m} + \cdots + u_0 k^{m+1} + d_m k^m + d_{m-1} k^{m-1} + \cdots + d_1 k + d_0 \\ 
        & = [u]_k \cdot k^{|\omega|} + [\omega]_k \\ 
        & = \alpha + \beta
    \end{align*}
    In general, we have 
    \begin{align*}
