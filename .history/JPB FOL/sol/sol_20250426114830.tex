...
\begin{solution}
    Suppose for a contradiction that $L$ is a regular language, thus there exists pumping lenght $p$ such that satisfies the pumping lemma. Now consider the string 
    \[ \omega := 0^p 1^p \in L \]
    of length $2p$. For this string, in its decomposition $\omega = xyz$, we have $xy$ is made of purely zero's. It is now easy to see that 
    \[ x y^i z \not L \]
    for, for example, $i = 2$. This contradicts our assumption that $L$ is regular, hence $L$ is not 
\end{solution}
